...
def menu():
    os.system('clear')
    print (logo4)
    print ('     \x1b[1;92m            Auto create tool  \x1b[97;1m  ')
    print('──────────────────────────────────────────────────')
    print (' └─[01] Auto Create Facebook')
    print (' └─[00] Auto React Soon')
    print (' └─[00] Auto Followers Soon ')
    print('──────────────────────────────────────────────────')
    sel = input(' └─[•] Select: ')
    if sel in['1', '01']:
        create().start()
        ...
        for x in range(lim):
            ...
            try:
                ...
                if 'checkpoint' in getts.url:
                    ...
                    print ('\r\033[1;33m[ Un-Create ] '+cok['c_user']+' | '+passw+'\033[0;97m     ')
                        ...
                        for x in BeautifulSoup(po.text, 'html.parser').find_all('form', {'method': 'post'}):
                            if 'confirmation_event_location=cliff' in str(x):
                                ...
                                if 'checkpoint' in rex.url:
                                    ...
                                    cok = self.ses.cookies.get_dict()
                                    print ('\r\033[1;32m[ Create-Done ] '+cok['c_user']+' | '+passw+'\033[0;97m     ')
